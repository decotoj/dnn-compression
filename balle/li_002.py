...
AWS = 1 #Flag to denote training on AWS or not

#Tunable Hyperparameters
LMBDA = 0.01#0.01  #Relative Importance of Distortion vs Rate Loss (Higher Value Weights Distortion Loss More Heavily)
LEARNING_RATE = 1e-4 #Nominally 1e-4
MASK_THRESHOLD = 0.1 #Nominally 0.1
RATE_LOSS_THRESHOLD = 100000 #Point Below Which Rate Loss is Set to Zero (Used to Govern Approx Compression Ratio)

#Constants (Including Hyperparameters Treated as Constants)
MODEL_DIRECTORY = 'model/li_002'  # Relative location where model files will be saved
...
def train():
  ...
  E, fx = encoder_li(x) #E is equivalent of y in Balle
  P = importance_map(fx)
  M = gen_mask(P)

  entropy_bottleneck = tfc.EntropyBottleneck()
  E_tilde, likelihoods = entropy_bottleneck(E, training=True)
  B = binarizer(E_tilde)

  bc = tf.multiply(B,M)
  x_tilde = decoder_li(bc)
  ...
  # #Rate Loss
  #rateLoss = rate_loss(P)
  rateLoss = tf.reduce_sum(P) / num_pixels#TEST1234
  rateLoss2 = tf.reduce_sum(tf.log(likelihoods)) / (-np.log(2) * num_pixels)
  ...
  train_loss = LMBDA * train_mse + rateLoss + rateLoss2 #TEST1234
  ...
  # ETstats = [tf.math.reduce_min(E_tilde), tf.math.reduce_max(E_tilde), tf.reduce_sum(E_tilde)]
  # Mstats = [tf.math.reduce_min(M), tf.math.reduce_max(M),tf.reduce_sum(M)]
  # Bstats = [tf.math.reduce_min(B), tf.math.reduce_max(B),tf.reduce_sum(B)]
  # BCstats = [tf.math.reduce_min(bc), tf.math.reduce_max(bc), tf.reduce_sum(bc)]
  ...
  with tf.train.MonitoredTrainingSession(scaffold=scaffold, hooks=hooks, checkpoint_dir=MODEL_DIRECTORY,
    ...
    while not sess.should_stop():
      ...
      if epSub >= LOG_STEPS:
        ...
        ep += 1
      if epSub == 0:
        print(ep*LOG_STEPS+epSub, 'TRAIN/DIST/RATE LOSS:', sess.run(train_loss), sess.run(train_mse), sess.run(rateLoss), sess.run(rateLoss2))
        # print('    Pstats', sess.run(Pstats))
        # print('    ETstats', sess.run(ETstats))
